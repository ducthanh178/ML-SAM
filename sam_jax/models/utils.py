...
# Note: activation() function is not used in Linen models
# Models use nn.BatchNorm and nn.relu directly
# This function is kept for backward compatibility but not recommended for new code
def activation(x: jnp.ndarray,
  ...
  NOTE: In Flax Linen, this function is not recommended.
  Use nn.BatchNorm and nn.relu directly in your model.
  """
  # This is a compatibility function - not ideal for Linen
  # Models should use nn.BatchNorm directly
  raise NotImplementedError(
      "activation() is not compatible with Flax Linen. "
      "Use nn.BatchNorm and nn.relu directly in your model.")
...
def shake_shake_train(xa: jnp.ndarray,
  ...
  """
  if rng is None:
    raise ValueError("rng must be provided in linen API")
